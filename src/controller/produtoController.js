...
module.exports = {
    ...
    async add(req,res){
        ...
        await Produto.create({nome,valor,tipo}).then((produto) => {
            req.flash('msg', produto.nome + 'Sucesso ao adicionar o produto!');
            res.redirect('admin/produto/add')
        }, (err) => {
            req.flash('msg', "Problema para adicionar produto! ")
            res.redirect('/admin/produto/add')
        })
    },
    ...
    async edit(req,res){
        ...
        produto.save().then(
            (produto) => {
                req.flash('msg', 'Problema na alteração o produto!');
                res.render('admin/produto/edit.ejs', {'Produto':produto, 'msg':req.flash('msg')})
            },
            (err) => {
                req.flash('msg', produto.nome + 'Alteração do produto realizada com sucesso!');
                res.render('admin/produto/edit.ejs', {'Produto':produto, 'msg':req.flash('msg')})
            }
        );
    },
    async del(req,res){
        ...
        await Produto.destroy({
            ...
            (err) => {
                req.flash('msg', 'Problema para deletar o produto!');
                res.redirect('admin/produto/')
            }
